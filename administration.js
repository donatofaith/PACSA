...
async function loadAuditLog(){const body=A$('auditTableBody');if(!body)return;const {data,error}=await supabaseClient.from('audit_logs').select('id,actor_role,action,entity_type,entity_id,created_at').order('created_at',{ascending:false}).limit(30);if(error){body.innerHTML=`<tr><td colspan="5" class="empty-inline">${aEsc(error.message)}</td></tr>`;return;}body.innerHTML=(data||[]).length?(data||[]).map(row=>`<tr><td>${aEsc(formatTime(row.created_at))}</td><td>${aEsc(row.actor_role||'system')}</td><td>${aEsc(String(row.action||'').replaceAll('_',' '))}</td><td>${aEsc(row.entity_type||'--')}</td><td>${aEsc(row.entity_id||'--')}</td></tr>`).join(''):'<tr><td colspan="5" class="empty-inline">No security activity recorded yet.</td></tr>';}

async function createAdmin(){const fullname=A$('adminFullname').value.trim();const email=A$('adminEmail').value.trim().toLowerCase();clearBox('adminMessage');A$('adminCredentials').className='credentials-box';A$('adminCredentials').textContent='';if(!fullname||!email)return showBox('adminMessage','Enter the administrator name and email.','error');const button=A$('createAdminBtn');button.disabled=true;button.textContent='Creating...';try{const data=await invokeAdminFunction({action:'create',fullname,email});showBox('adminMessage',data?.email_sent?'Administrator created. Login details were emailed.':'Administrator created. Email could not be sent, so copy the temporary password below.');A$('adminCredentials').classList.add('show');A$('adminCredentials').textContent=`Admin: ${fullname}\nEmail: ${email}\nTemporary Password: ${data?.temporary_password||'--'}\nExpires: ${data?.temporary_password_expires_at||'--'}\nLogin: ${location.origin}/admin-login.html`;A$('adminFullname').value='';A$('adminEmail').value='';await Promise.all([loadAdmins(),loadAuditLog()]);}catch(e){A$('adminCredentials').className='credentials-box';A$('adminCredentials').textContent='';showBox('adminMessage',e?.message||'Could not create administrator.','error');}finally{button.disabled=false;button.textContent='Create Admin';}}
