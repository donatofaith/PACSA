...
// Always return readable JSON. The frontend can show the real reason instead of
// the generic "Edge Function returned a non-2xx status code" message.
const fail = (message: string, code = "ADMIN_ACTION_FAILED") =>
  json({ ok: false, error: message, code });
      ...
      htmlContent: `
        <div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden">
          ...
          <div style="padding:24px;line-height:1.6">
            ...
            <p>This temporary password expires in 24 hours. You will be required to create your own strong password after signing in.</p>
...
Deno.serve(async req => {
  ...
  try {
    ...
    const { data: superAdmin, error: roleError } = await adminClient
      .from("admins")
      .select("id,fullname,email,role,status,auth_user_id")
    ...
    if (
      roleError ||
      !superAdmin ||
      norm(superAdmin.status || "active") !== "active" ||
      norm(superAdmin.role) !== "super_admin"
    ) {
      return fail("Super Admin access is required.", "SUPER_ADMIN_REQUIRED");
    }
    ...
    if (action === "delete") {
      ...
      if (target.auth_user_id) {
        ...
        if (authDeleteError) return fail(`Could not remove the Admin login: ${authDeleteError.message}`, "AUTH_DELETE_FAILED");
      }
      ...
      await adminClient.from("audit_logs").insert({
        ...
        details: { fullname: target.fullname, email: target.email },
      });
    ...
    if (action === "set_status") {
      ...
      if (!adminId || !["active", "inactive"].includes(status)) {
        return fail("Administrator and a valid status are required.", "INVALID_STATUS_REQUEST");
      }
      ...
      const { error } = await adminClient.from("admins").update({ status }).eq("id", adminId);
      if (error) return fail(error.message, "STATUS_UPDATE_FAILED");
    ...
    if (createError || !created?.user) {
      const text = createError?.message || "Could not create Admin login.";
      if (norm(text).includes("already") || norm(text).includes("registered")) {
        return fail("This email already has a Supabase login. Use another email or remove the old test login from Authentication first.", "AUTH_EMAIL_EXISTS");
      }
      return fail(text, "AUTH_CREATE_FAILED");
    }
    ...
    if (insertError) {
      ...
      return fail(insertError.message || "Could not create Admin record.", "ADMIN_ROW_CREATE_FAILED");
    }
    ...
    await adminClient.from("audit_logs").insert({
      ...
      details: { fullname, email, email_sent: emailSent },
    });
    ...
    return json({
      ...
      message: emailSent
        ? "Administrator created and login details were emailed."
        : "Administrator created. Email could not be sent, so copy the temporary password manually.",
    });
  } catch (error) {
