-- PACSA published report visibility hotfix
-- Makes student report RPCs independent of underlying ID column types.
...
as $$
  select
    ...
    r.session::text,
    r.remark::text,
    coalesce(r.teacher_remark,r.remark)::text,
  ...
  where s.auth_user_id = auth.uid()
    and lower(trim(coalesce(r.status::text,''))) = 'published'
