-- PACSA secure teacher result deletion - UUID result IDs
-- Run after 20260918_teacher_result_edit_delete.sql if that migration was already applied.

begin;

drop function if exists public.pacsa_teacher_delete_result(bigint);
drop function if exists public.pacsa_teacher_delete_result(uuid);

create function public.pacsa_teacher_delete_result(p_result_id uuid)
returns boolean
language plpgsql
security definer
...
revoke all on function public.pacsa_teacher_delete_result(uuid) from public;
grant execute on function public.pacsa_teacher_delete_result(uuid) to authenticated;
