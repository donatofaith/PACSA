...
(function(){
  ...
  function ensureStyles(){
    ...
    style.textContent=`
      ...
      .pacsa-temp-head{background:linear-gradient(135deg,#4C1D95,#7C3AED);color:#fff;padding:24px}
      .pacsa-temp-head h2{margin:0 0 7px}.pacsa-temp-head p{margin:0;opacity:.9;line-height:1.5;font-size:14px}
      ...
      .pacsa-temp-help{font-size:12px;color:#6b7280;line-height:1.5;margin:-3px 0 15px}.pacsa-temp-msg{display:none;padding:11px 12px;border-radius:10px;margin-bottom:14px;font-size:13px}.pacsa-temp-msg.show{display:block}.pacsa-temp-msg.error{background:#fee2e2;color:#b91c1c}.pacsa-temp-msg.success{background:#dcfce7;color:#166534}
  ...
  async function ensure(user,options={}){
    ...
    return await new Promise(resolve=>{
      ...
      overlay.className='pacsa-temp-overlay';
      const role=esc(options.role||'PACSA');
      ...
      const msg=overlay.querySelector('#pacsaTempMsg');
      const show=(text,type='error')=>{msg.textContent=text;msg.className=`pacsa-temp-msg show ${type}`;};
      overlay.querySelector('#pacsaTempSignout').onclick=async()=>{try{await supabaseClient.auth.signOut();}catch{} overlay.remove();resolve(false);};
      overlay.querySelector('#pacsaTempSave').onclick=async event=>{
        const button=event.currentTarget;
        const password=overlay.querySelector('#pacsaTempNew').value;
        const confirmPassword=overlay.querySelector('#pacsaTempConfirm').value;
        ...
        try{
          const {error}=await supabaseClient.auth.updateUser({password,data:{temporary_password:false,temporary_password_expires_at:null,password_changed_at:new Date().toISOString()}});
          if(error)throw error;
          show('Password changed successfully.','success');
          setTimeout(()=>{overlay.remove();resolve(true);},650);
        }catch(error){show(error?.message||'Could not change password.');button.disabled=false;button.textContent='Change Password';}
      };
    });
  }
