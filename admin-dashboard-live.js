...
const PACSA_LIVE = (() => {
    ...
    const getTotal = result => {
        ...
        return (Number.isFinite(ca) ? ca : 0) + (Number.isFinite(exam) ? exam : 0);
    };

    const fullName = student =>
        `${student?.first_name || ""} ${student?.last_name || ""}`.trim() ||
        student?.fullname || student?.student_id || "Student";
    ...
    function injectStyles() {
        ...
        style.textContent = `
            .notification-btn{position:relative}.notification-dot.hidden{display:none!important}.pacsa-notification-count{position:absolute;top:-7px;right:-7px;min-width:18px;height:18px;padding:0 5px;border-radius:999px;background:#dc2626;color:#fff;font-size:11px;font-weight:800;display:none;align-items:center;justify-content:center;border:2px solid #fff}.pacsa-notification-panel{position:fixed;top:82px;right:28px;width:min(390px,calc(100vw - 32px));max-height:min(560px,calc(100vh - 105px));overflow-y:auto;background:#fff;border:1px solid #e5e7eb;box-shadow:0 18px 45px rgba(15,23,42,.16);border-radius:18px;padding:16px;z-index:9999;display:none}.pacsa-notification-panel.active{display:block}.pacsa-notification-header{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px}.pacsa-notification-header h3{margin:0;font-size:16px}.pacsa-notification-header a{color:#5b21b6;font-weight:700;text-decoration:none;font-size:13px}.pacsa-notification-item{display:flex;gap:10px;padding:12px 0;border-top:1px solid #f1f5f9;text-decoration:none}.pacsa-notification-icon{width:34px;height:34px;border-radius:12px;display:flex;flex:0 0 auto;align-items:center;justify-content:center;background:#f3e8ff}.pacsa-notification-body strong{display:block;font-size:14px;color:#111827}.pacsa-notification-body span{display:block;margin-top:3px;color:#64748b;font-size:12px;line-height:1.4}.pacsa-notification-empty{padding:16px 0 6px;color:#64748b;font-size:14px}@media(max-width:640px){.pacsa-notification-panel{top:78px;right:16px;left:16px;width:auto}}
        `;
    ...
    function makeNotifications({ applications, results, studentReports }) {
        ...
        results
            ...
            .forEach(result => notices.push({
                icon: "📑",
                title: "Result activity",
                detail: `${result.subject || "Subject result"} for ${result.class || "a class"} has been entered and is progressing through the result workflow`,
        ...
        studentReports
            ...
            .forEach(report => notices.push({
                icon: "📊",
                title: "Report awaiting Principal",
                detail: `${report.student_id || "A student"} report has a Class Teacher submission and is waiting for Principal approval`,
    ...
    function updateNotificationBell(data) {
        ...
        const unread = notices.filter(notice => !lastSeen || new Date(notice.date || 0) > new Date(lastSeen));
    ...
    function buildNotificationPanel(notices, latestCreatedAt) {
        ...
        panel.innerHTML = `
            <div class="pacsa-notification-header"><h3>Notifications</h3><a href="results.html">Monitor Results</a></div>
            ${items.length ? items.map(notice => `
                <a class="pacsa-notification-item" href="${escapeHtml(notice.href || "#")}">
                    ...
                    <div class="pacsa-notification-body"><strong>${escapeHtml(notice.title)}</strong><span>${escapeHtml(notice.detail)} • ${escapeHtml(timeAgo(notice.date))}</span></div>
                </a>`).join("") : `<div class="pacsa-notification-empty">No important admin notifications right now.</div>`}
        `;
        const button = document.querySelector(".notification-btn");
        if (!button.dataset.pacsaNoticeBound) {
            ...
            document.addEventListener("click", event => {
                if (panel.classList.contains("active") && !panel.contains(event.target) && !button.contains(event.target)) panel.classList.remove("active");
            });
        }
    }
    ...
    function updateDashboardAnalytics(results, students) {
        const usable = results
            .filter(result => !["draft", "pending", "rejected"].includes(norm(result.status || "published")))
            .map(result => ({ ...result, _total: getTotal(result) }))
        ...
        const schoolAverage = Math.round(usable.reduce((sum, result) => sum + result._total, 0) / usable.length);
        const passRate = Math.round((usable.filter(result => result._total >= 40).length / usable.length) * 100);
        ...
        Object.entries(byClass).forEach(([className, totals]) => {
            ...
            if (average > bestClassAverage) { bestClassAverage = average; bestClass = className; }
        });
        ...
        Object.entries(byStudent).forEach(([studentId, totals]) => {
            const average = totals.reduce((sum, score) => sum + score, 0) / totals.length;
            if (average > topAverage) { topAverage = average; topStudentId = studentId; }
        });
        const topStudent = students.find(student => String(student.student_id) === String(topStudentId));
