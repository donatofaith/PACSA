...
const resetUrl = () =>
    new URL("student-reset-password.html", window.location.href).href;
...
function isTemporaryPasswordExpired(user) {
    const expiry = user?.user_metadata?.temporary_password_expires_at;
...
$("studentLoginForm")?.addEventListener("submit", async event => {
    ...
    if (!email || !password) {
        showMessage("Enter your registered email and password.");
        return;
    }
    ...
    try {
        ...
        if (!authData?.user) throw new Error("Student login failed.");

        if (isTemporaryPasswordExpired(authData.user)) {
            await supabaseClient.auth.signOut();
            throw new Error("This temporary password has expired. Please ask Admin to create a new temporary password.");
        }
        ...
        if (!student) {
            await supabaseClient.auth.signOut();
            throw new Error("This account is not linked to a PACSA student.");
        }

        if (norm(student.email) !== norm(email)) {
            await supabaseClient.auth.signOut();
            throw new Error("This email does not match the student record.");
        }

        if (norm(student.status || "active") !== "active") {
            await supabaseClient.auth.signOut();
            throw new Error("Your student record is inactive.");
        }

        if (norm(student.portal_status) !== "active") {
            await supabaseClient.auth.signOut();
        ...
        if (text.includes("invalid login credentials")) {
            ...
            showMessage(error?.message || "Could not login.");
        }
