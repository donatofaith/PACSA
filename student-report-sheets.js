...
async function renderMidtermSheet(){
  ...
  host.innerHTML=`<div class="official-sheet-inner">${schoolHead()}<div class="sheet-title">Senior School Mid-Term Performance Report Sheet</div><table class="sheet-info"><tr><td><b>NAME:</b> ${sEsc(getStudentName())}</td><td><b>CLASS:</b> ${sEsc(latest.class)}</td><td><b>AGE:</b> ${sEsc(meta?.age??'--')}</td><td><b>SESSION:</b> ${sEsc(latest.session)}</td><td><b>TERM:</b> ${sEsc(latest.term)}</td></tr><tr><td colspan="3"></td><td><b>NO. IN CLASS:</b> ${sEsc(meta?.number_in_class??'--')}</td><td><b>POSITION:</b> ${sOrdinal(meta?.class_position)}</td></tr></table><table class="sheet-table"><thead><tr><th>Subject</th><th>1st Test<br>(10)</th><th>2nd Test<br>(20)</th><th>Total<br>(30)</th><th>Position</th><th>Remarks</th><th>Signature</th></tr></thead><tbody>${body.join('')}</tbody></table><table class="sheet-summary"><tr><td>SUBJECTS OFFERED: ${rows.length}</td><td>MARKS OBTAINABLE: ${obtainable}</td><td>MARKS OBTAINED: ${marks}</td><td>% OF MARK: ${percent.toFixed(1)}%</td></tr></table><table class="sheet-comments"><tr><td>Class Teacher’s Comment</td><td>${sEsc(meta?.teacher_remark||'--')}</td></tr><tr><td>Principal’s Comment</td><td>${sEsc(meta?.principal_remark||'--')}</td></tr></table><table class="sheet-key"><tr><td>KEY</td><td>A: 30 EXCELLENT, B: 23–29 VERY GOOD, C: 15–22 GOOD, D: 10–14 FAIR, F: 0–9 POOR</td></tr></table><div class="sheet-actions"><button class="sheet-print-btn" id="printMidtermBtn" type="button">Print Mid-Term Report</button></div></div>`;
...
async function renderFinalSheet(){
  ...
  const list=rows||[], marks=list.reduce((a,r)=>a+(Number(r.total)||0),0), obtainable=list.length*100, percent=obtainable?marks/obtainable*100:0;
  const body=[];
